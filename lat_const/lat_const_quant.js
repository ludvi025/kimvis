...
function submitOptions(){
	...
	options = {
		...
		xerr: '',
		xaxis: 'Temperature',
		yaxis: 'Lattice Constant' + ' ( ' + d3.select('#latconst-units').property('value') + ' )', 
		xunits: '',
		yunits: d3.select('#latconst-units').property('value'),
	...
	data = data.filter(function(d){
		...
		if (!inRange(d[options.y], range)){
			keep = false;
			console.log(false);
		}

		if (!filters.isSelected(d)){	// Checks if the item passes all the filters
			keep = false;
		}
		
		// Show ref. data, test results, or both
		if ( rdtr != 'b' ){
			if ( d['source'] != rdtr ){
				keep = false;
			}
		...
		if (options.yunits == 'nm'){ // Dirty units fix
				d['a']/=10;
				d['b']/=10;
				d['c']/=10;
		}
	...
	if (plot){
		...
		plot = new scatterPlotQuantitative(data, options);
...
function mouseUp(){
	if (zoom_box_down){
		...
		var x_min = plot.xScale.invert(Number(xRange.attr('x1'))-plot.margin.left);
		var x_max = plot.xScale.invert(Number(xRange.attr('x2'))-plot.margin.left);
